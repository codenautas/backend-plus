...
function tableDefAdapt(tableDef, context){
    ...
    if(context){
        resultTableDef.foreignKeys.forEach(function(fkDef){
            ...
            resultTableDef.sql.from += '\n    left join ' + fkTableDef.sql.from + ' on ' +
            ...
            var iLastSourceField = fkTableDef.fields.findIndex(function(fieldDef){ return fieldDef.name == lastSourceField });
            fkTableDef.nameFields.forEach(function(fieldName){
                var fieldNameAdded=fkTableDef.alias+'_'+fieldName;
                ...
                resultTableDef.sql.select.push(
                    be.db.quoteObject(fkTableDef.alias)+'.'+be.db.quoteObject(fieldName)+' as '+
                    be.db.quoteObject(fkTableDef.alias+'_'+fieldName)
                );
            });
        });
    }else{
