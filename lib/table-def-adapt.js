...
function tableDefAdapt(tableDef){
    ...
    resultTableDef.fields = resultTableDef.fields.map(function(fieldDef){
        ...
        if(fieldDef.typeName==='bigint'){
            resultFieldDef.typeName='number';
            resultFieldDef.exact=true;
            resultFieldDef.decimals=0;
            resultFieldDef.sizeByte=8;
        }
    ...
    resultTableDef.foreignKeys.map(function(fk){
        fk.fields.map(function(pair, i, pairs){
            if(typeof pair === "string"){
                pairs[i] = {source: pair, target: pair};
            }
        });
    });
    console.log('xxxxxxxxxxxxx tda',resultTableDef.name);
    console.dir(resultTableDef.foreignKeys,{depth:8});
    resultTableDef.detailTables.map(function(detail){
        detail.fields.map(function(pair, i, pairs){
            if(typeof pair === "string"){
                pairs[i] = {source: pair, target: pair};
            }
        });
