...
function tableDefAdapt(tableDef, context){
    ...
    resultTableDef.fields = resultTableDef.fields.map(function(fieldDef){
        ...
        return fieldDef.allow.select || context.forDump;
    ...
    if(resultTableDef.functionDef){
        resultTableDef.sql.tableName+=
        console.log('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',resultTableDef.sql.tableName)
    }
    ...
    resultTableDef.sql.from = (resultTableDef.sql.from || (
        be.db.quoteObject(resultTableDef.tableName)+(resultTableDef.functionDef?
    ...
    if(context && !context.plainStructure){
        resultTableDef.foreignKeys.concat(resultTableDef.softForeignKeys).forEach(function(fkDef){
            ...
            fkDef.displayFields = fkDef.displayAllFields?
                (fkTableDef.fields.filter(fd=>fd.visible && !fd.clientSide && !resultTableDef.field[fd.name]).map(fd=>fd.name)):
            ...
            if(fkDef.fields.length===1){
                if(fkTableDef.field[fkDef.fields[0].target].isPk){
                    try{
                        console.log('xxxxxxxxxxxxxx fk',fkDef.fields[0].source,fkDef.skipReferenceLookup, fkTableDef.skipReferenceLookup);
