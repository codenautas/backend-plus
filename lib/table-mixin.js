...
module.exports = function(tableName1, tableName2, tableDefForMixin){
    return function(context){
        ...
        tableDef.sql.from = td1.sql.from+" inner join "+td2.sql.from+" on "+joinDef.fields.map(function(pair){
            return context.be.db.quoteIdent(td1.name)+"."+pair.source+" = "+context.be.db.quoteIdent(td2.name)+"."+pair.target;
        }).join(" AND ");
        tableDef.fields = tableDef.fields.concat(otherColumns);
        tableDef.sql.select = [context.be.db.quoteIdent(td1.name)+".*"].concat(otherColumns.map(function(fieldDef){
            return context.be.db.quoteIdent(td2.name)+"."+context.be.db.quoteIdent(fieldDef.name);
        }));
