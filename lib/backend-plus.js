...
AppBackend.prototype.exts = {
    ...
    normal: ['', 'js', 'map', 'html', 'css', 'jpg', 'jpeg', 'png', 'ico', 'gif', 'appcache', 'manifest', 'json', 'webmanifest']
};
    ...
    return Promise.resolve().then(function(){
        ...
        if(config.nodb){
            ...
            return Promise.resolve().then(function(){
                ...
                be.dbUserRolExpr=`(select ${be.db.quoteIdent(be.config.login.rolFieldName)} from ${be.db.quoteIdent(be.config.login.table)} where ${be.db.quoteIdent(be.config.login.rolFieldName)} = ${be.dbUserNameExpr})`
            }).then(function(){
                                    ...
                                    if(progressInfo.lengthComputable){
                                        if(new Date()-(context.lastMessageSended||0) > 500 || progressInfo.force){
                                            ...
                                            if(progressInfo.total){
                                                if(Math.floor(progressInfo.loaded*100/progressInfo.total)<1){
                                                    progress2send={message:'('+Math.floor(progressInfo.loaded*1000/progressInfo.total)+'‰)', ephemeral:true};
                                                }else{
                                                    progress2send={message:Math.floor(progressInfo.loaded*100/progressInfo.total)+'%', ephemeral:true};
                                                }
                                            }
                                        }else{
                                            progress2send=null;
        ...
        try{
            ...
            if(tableDef.sql.isTable){
                ...
                if(tableDef.sql.policies.enabled){
                    policyLines.push(`ALTER TABLE ${db.quoteIdent(tableName)} ENABLE ROW LEVEL SECURITY;`);
                    ['all', 'select', 'insert', 'update', 'delete'].forEach((command)=>{
                        var polcom=tableDef.sql.policies[command];
                        if(polcom.using || polcom.check){
                            policyLines.push(`CREATE POLICY bp_${command} ON ${db.quoteIdent(tableName)} AS PERMISSIVE FOR ${command}`+
                ...
                if(tableDef.sql.viewBody){
                    lines.push('create or replace view '+db.quoteIdent(tableDef.sql.tableName)+' (');
                    var viewFields=[];
                    tableDef.fields.forEach(function(fieldDef){
                    ...
                    lines.push('grant select on '+db.quoteIdent(tableDef.sql.tableName)+' to '+user+';');
                    lines.push('');
                }
            }
        }catch(err){
    ...
    return Promise.all(Object.keys(partialTableStructures).map(function(tableName){
        ...
        return locatePath(buscarEn).then(function(theTableFileName){
            ...
            var filterField=function(fieldName){
                ...
                return defField.inTable!==false && (!defField.clientSide || defField.serverSide) && !defField.generatedAs;
            }
