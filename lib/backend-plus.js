...
AppBackend.prototype.dumpDb = function dumpDb(){
    ...
    for(var tableName in be.tableStructures){
        ...
        tableDef.fields.forEach(function(fieldDef){
            fields.push('  '+db.quoteObject(fieldDef.name)+' '+(typeDb[fieldDef.typeName]||'"'+fieldDef.typeName+'"')+' '+(fieldDef.typeName.nullable?'NOT NULL ':''));
        });
        ...
        (tableDef.foreignKeys||[]).map(function(fk){
            lines.push('alter table '+db.quoteObject(tableDef.name)+' add foreign key ('+
    ...
    return fs.writeFile('local-db-dump.sql', lines.join('\n'));
};
