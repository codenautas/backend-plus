...
AppBackend.prototype.configList = function configList(){
    ...
    if(!this.staticConfig["client-setup"].title && fs.existsSync(this.rootPath+'/def-config.yaml')){
        ...
        console.log('ERROR el def-config hay que ponerlo dentro de staticConfig');
        console.log('DEPRECATED!!!!!!')
        console.log('DEPRECATED!!!!!!')
        console.log('DEPRECATED!!!!!!')
        console.log('DEPRECATED!!!!!!')
...
AppBackend.prototype.mainPage = function mainPage(req, offlineMode){
    ...
    if(offlineMode && req && req.user){
        ...
        metaAttr=changing(metaAttr,{name:"viewport", content:"width="+width+", initial-scale=1.0"});
    }
        ...
        try{
            ...
            if(tableDef.sql.isTable){
                ...
                tableDef.foreignKeys.map(function(fk){
                    ...
                    var consName=db.quoteIdent(fk.consName?fk.consName:(tableDef.alias||tableDef.sql.tableName)+' '+fk.references+' REL');
                    var indexName=db.quoteIdent(targetFieldList.replace(/[ "]/g,'')+' 4 '+tableDef.sql.tableName+' IDX');
                    ...
                    indexLines.push('create index '+indexName+' ON '+db.quoteIdent(tableDef.sql.tableName)+' ('+targetFieldList+');');
                });
