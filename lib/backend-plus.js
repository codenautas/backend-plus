...
AppBackend.prototype.clientIncludesCompleted = function clientIncludesCompleted(req) {
    let list = this.clientIncludes(req);
...
AppBackend.prototype.clientModules = function clientModules(req) {
    return { scripts: this.clientIncludesCompleted(req).filter(x => x.type == 'js').map(mod => {return { src: mod.src }})};
}
...
AppBackend.prototype.csss = function csss() {
    return this.clientIncludesCompleted().filter(x => x.type == 'css').map(x => x.href);
};
