        ...
        mainApp.use(function(req,res,next){
            ...
            if(be.config.devel){
                ...
                if(!be.seenBrowsers[jsonBro].times){
                    console.log('************ useragent',req.useragent.shortDescription,req.machineId,req.useragent.source);
                    fs.writeFile('local-browsers',JSON.stringify(be.seenBrowsers,null,'    ')).catch(function(err){
                        console.log('**********!!! ERR',err,req.useragent);
                    });
...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    skinPaths.concat(unloggedPaths).forEach(function(path){
        ...
        console.log('----------- serving',baseUrl+(skin?'/'+skin:''),pathToNodeModulesSkin);
    ...
    unloggedPaths.forEach(function(path){
        var pathToNodeModulesSkin=Path.join(path, 'unlogged');
        console.log('----------- Serving',baseUrl,pathToNodeModulesSkin,optsGenericForFilesUnlogged);
    ...
    console.log('------------ be.rootPath', be.rootPath);
    console.log('------------ Path.resolve(be.rootPath)', Path.resolve(be.rootPath));
    console.log('------------ skinPaths', skinPaths);
    console.log('------------ unloggedPaths', unloggedPaths);
};
    ...
    for(var tableName in be.tableStructures){
        ...
        if(tableDef.sql.isTable){
            ...
            tableDef.constraints.map(function(cons){
                ...
                consLines.push('alter table '+db.quoteIdent(tableDef.name)+' add '+sql+';');
            });
