...
AppBackend.prototype.dumpDb = function dumpDb(){
    ...
    for(var tableName in be.tableStructures){
        ...
        tableDef.fields.forEach(function(fieldDef){
            fields.push('  '+fieldDef.name+' '+(typeDb[fieldDef.typeName]||'"'+fieldDef.typeName+'"')+' '+(fieldDef.typeName.nullable?'NOT NULL ':''));
        });
        lines.push(fields.join(', \n'));
        if(tableDef.primaryKey){
            lines.push(', primary key ('+tableDef.primaryKey.join(', ')+')');
        }
        lines.push(');');
        lines.push('');
    }
