...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    var pathToNodeModulesSkin=Path.join(be.config.server.skins[skin]['local-path'],skin,'unlogged');
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    this.app.use(serveContent(Path.resolve(be.config.server.skins[skin]['local-path']),{allowedExts:be.exts.img.concat(['css'])}));
