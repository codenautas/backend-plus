...
AppBackend.prototype.unloggedLandPage = function unloggedLandPage(req){
    ...
    return html.div([
        ...
        html.h2({style:'text-align:center'},[html.code([
            html.a({id:'goto-login', style:'border:0.5px solid blue; border-radius: 6px; padding: 6px', href:be.config.login.plus.loginUrlPath||'/login'},'login')
        ])]),
...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    var be=this;
    if(be.config.login.unloggedLandPage){
        mainApp.get(baseUrl+(baseUrl?'/':'')+be.config.login.unloggedLandPage, function(req, res, next){
            res.status(401);
