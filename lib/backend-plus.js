...
AppBackend.prototype.start = function start(opts){
    ...
    return Promise.resolve().then(function(){
        ...
        await be.addProcedureServices(true, mainApp);
        if(be.config.nologin){
...
AppBackend.prototype.addProcedureServices = function addProcedureServices(forUnlogged, mainApp){ 
    var be = this;
    if(forUnlogged){
        var app = express();
    }else{
    ...
    return this.getProcedures().then(function(defs){
        ...
        if(forUnlogged){
            mainApp.use(be.config.server["base-url"],app);
        }
    });
};
