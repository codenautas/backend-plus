...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    console.log('xxxxxxxxxxxx',resolve_module_dir('require-bro'));
    console.log('xxxxxxxxxxxx',resolve_module_dir('dialog-promise'));
    console.log('xxxxxxxxxxxx',resolve_module_dir('xlsx-style','dist'));
    console.log('xxxxxxxxxxxx',resolve_module_dir('js-yaml','lib'));
    be.clientIncludes().filter(x => x.module).forEach(function (moduleDef) {
        mainApp.use(baseUrl + (moduleDef.path ? '/' + moduleDef.path : ''), serveContent(resolve_module_dir(moduleDef.module, moduleDef.pathMod), { allowedExts: moduleDef.type }));
    })
    mainApp.use(baseLib, serveContent(resolve_module_dir('dialog-promise'), {allowedExts:['js','css']}));
    mainApp.use(baseUrl+'/css', serveContent(resolve_module_dir('dialog-promise'), {allowedExts:['css']}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('js-to-html'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('ajax-best-promise'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('lazy-some'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('like-ar'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('json4all'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('js-yaml','dist'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('typed-controls'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('best-globals'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('regexplicit'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('castellano'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('big.js'), {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('type-store'), {allowedExts:'js'}));
    // mainApp.use(baseLib, serveContent('./node_modules/xlsx/dist', {allowedExts:'js'}));
    mainApp.use(baseLib, serveContent(resolve_module_dir('xlsx-style','dist'), {allowedExts:'js'}));
    mainApp.use(baseUrl+'/pikaday', serveContent(resolve_module_dir('pikaday'), {allowedExts:['js','css']}));
    // ----------------------------------------------------
...
AppBackend.prototype.clientIncludes = function clientIncludes(req) {
    var be = this;
    var list = [
        { type: 'js', src: 'lib/js-yaml.js' },
        { type: 'js', src: 'lib/xlsx.core.min.js' },
        { type: 'js', src: 'require-bro.js', path: 'lib', module: 'require-bro', pathMod: '' },
        { type: 'js', src: 'lib/lazy-some.js' },
        { type: 'js', src: 'lib/like-ar.js' },
        { type: 'js', src: 'lib/best-globals.js' },
        { type: 'js', src: 'lib/json4all.js' },
        { type: 'js', src: 'lib/postgres-interval4client.js' },
        { type: 'js', src: 'lib/dialog-promise.js' },
        { type: 'js', src: 'lib/js-to-html.js' },
        { type: 'js', src: 'pikaday/pikaday.js' },
        { type: 'js', src: 'lib/big.js' },
        { type: 'js', src: 'lib/type-store.js' },
        { type: 'js', src: 'lib/typed-controls.js' },
        { type: 'js', src: 'lib/ajax-best-promise.js' },
        { type: 'js', src: 'my-things.js' },
        { type: 'js', src: 'my-skin.js' },
        { type: 'js', src: 'my-tables.js' },
        { type: 'js', src: 'my-inform-net-status.js' },
        { type: 'css', src: "css/dialog-promise.css" },
        { type: 'css', src: "css/my-things.css" },
        { type: 'css', src: "css/my-tables.css" },
        { type: 'css', src: "css/my-menu.css" },
        { type: 'css', src: "pikaday/css/pikaday.css" },
    ];
    if (req) {
    ...
    var config = be.config["client-setup"];
    if (config.menu === true) {
        list.push({ type: 'js', src: 'my-menu.js' });
    }
    if (config.lang === 'es') {
        list.push({ type: 'js', src: 'lib/cliente-en-castellano.js' });
    }
...
AppBackend.prototype.clientModules = function clientModules(req) {
    return { scripts: this.clientIncludes(req).filter(x => x.type == 'js').map(x => {return { src: (x.path ? x.path + '/' : '') + x.src }}) };
}
...
AppBackend.prototype.csss = function csss() {
    return this.clientIncludes().filter(x => x.type == 'css').map(x => (x.path ? x.path + '/' : '') + x.src);
};
    ...
    for(var tableName in be.tableStructures){
        ...
        if(tableDef.sql.isTable){
            ...
            if(tableDef.primaryKey){
                ...
                enanceLines.push(
                    'select enance_table('+db.quoteLiteral(tableDef.name)+','+db.quoteLiteral(
            ...
            tableDef.foreignKeys.map(function(fk){
                fkLines.push('alter table '+db.quoteIdent(tableDef.name)+' add foreign key ('+
