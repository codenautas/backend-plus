...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    be.clientIncludes().filter(x => x.module).forEach(function (moduleDef) {
        let baseLib = baseUrl + '/' + (moduleDef.path ? moduleDef.path : 'lib');
        try {
...
AppBackend.prototype.clientIncludes = function clientIncludes(req) {
    ...
    var list = [
        ...
        { type: 'js' , module: 'big.js', file: 'big.js' },
        
        { type: 'css', module: 'dialog-promise', path:'dialog-promise' },
        { type: 'css', module: 'pikaday', path:'pikaday' },
        ...
        { type: 'js' , module: 'js-yaml', modPath:'dist' },
        { type: 'js' , module: 'xlsx-style', modPath:'dist', file: 'xlsx.core.min.js' },
        { type: 'js' , module: 'regexplicit', modPath:'dist' },
        { type: 'js' , module: 'require-bro'},
        { type: 'js' , module: 'postgres-interval' },
        
        { type: 'js' , src: 'my-things' },
        { type: 'js' , src: 'my-skin' },
        { type: 'js' , src: 'my-tables' },
        { type: 'js' , src: 'my-inform-net-status' },
    ...
    if (req) {
        ...
        if (!browInfo || browInfo.polly === true || (req.useragent.version || '0').split('.')[0] < browInfo.polly) {
            list.unshift({ type: 'js', module: 'polyfills-bro' })
        }
    }
    var config = be.config["client-setup"];
    if (config.menu === true) {
        list.push({ type: 'js', src: 'my-menu' });
    }
    if (config.lang === 'es') {
        list.push({ type: 'js', module: 'castellano' });
    }
    list.push({ type: 'js', src: 'menu' });

    return list.map(inclution => Object.assign({
        src: inclution.type == 'js'?((inclution.path ? inclution.path : 'lib') + '/' + (inclution.file? inclution.file: (inclution.module + '.js'))):'',
        href: inclution.type == 'css'? ((inclution.path ? inclution.path : 'css') + '/' + inclution.module + '.css'):''
    },inclution));
}

AppBackend.prototype.clientModules = function clientModules(req) {
    return { scripts: this.clientIncludes(req).filter(x => x.type == 'js').map(mod => {return { src: mod.src }})};
}
...
AppBackend.prototype.csss = function csss() {
    return this.clientIncludes().filter(x => x.type == 'css').map(x => x.href);
};
