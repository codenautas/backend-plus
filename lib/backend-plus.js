...
AppBackend.prototype.start = function start(){
    ...
    return Promises.start(function(){
            ...
            be.getDbClient = function getDbClient(){
                return pg.connect(be.config.db).then(function(client){
                    ...
                    if(search_path.length>0){
                        console.log('PASE', "set SEARCH_PATH TO "+be.db.quoteObjectList(search_path));
        ...
        be.app.use(bodyParser.urlencoded({extended:true}));
        if(be.config.login){
            be.app.loginPlusManager = new loginPlus.Manager;
            be.app.loginPlusManager.init(be.app,be.config.login.plus);
            be.app.loginPlusManager.setValidator(
                function(username, password, done) {
                    be.getDbClient().then(function(client){
                        ...
                        done('internal login error');
                    });
                }
            );
            be.addLoggedServices();
        }
        be.app.use(function(req,res,next,err){
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    this.app.use('/',MiniTools.serveJade(this.rootPath+'client',true));
    this.app.use('/',MiniTools.serveStylus(this.rootPath+'client',true));
    this.app.use('/',extensionServeStatic(this.rootPath+'client',{staticExtensions:'js'}));
