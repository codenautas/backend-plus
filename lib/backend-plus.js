        ...
        return be.config.db && pg.connect(be.config.db).then(function(client){
            ...
            be.getDbClient = function getDbClient(req){
                return pg.connect(be.config.db).then(function(client){
                    var username = (req.user||{})[be.config.login.userFieldName]||'!logged';
                    console.log("SET application_name = "+be.db.quoteText(username+' '+req.machineId+' '+req.useragent.shortDescription))
                    return client.query(
                        "SET application_name = "+be.db.quoteText(username+' '+req.machineId+' '+req.useragent.shortDescription)
    ...
    return this.getProcedures().then(function(defs){
        ...
        be.procedures.forEach(function(procedureDef){
            ...
            be.app[procedureDef.method]('/'+procedureDef.action, function(req, res){
                ...
                var context={be, user:req.user, session:req.session};
                return Promises.start(function(){
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    this.app.use(extensionServeStatic('./node_modules/dialog-promise/lib', {staticExtensions:['js','css']}));
    ...
    this.app.use(extensionServeStatic('./node_modules/json4all', {staticExtensions:'js'}));
    this.app.use(extensionServeStatic('./node_modules/js-yaml/dist', {staticExtensions:'js'}));
    ...
    this.app.use(extensionServeStatic('./node_modules/castellano/lib', {staticExtensions:'js'}));
