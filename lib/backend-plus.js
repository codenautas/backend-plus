            ...
            be.app[procedureDef.method]('/'+procedureDef.action, function(req, res){
                ...
                var generateInsertSQL = function(insertElement){
                    ...
                    for (var key in insertElement) {
                        cleanKeys.push(be.db.quoteObject(key));
                        if(typeof(insertElement[key]) !== 'string'){
                            insertElement[key] = insertElement[key].toString();    
                        }
                        cleanValues.push(be.db.quoteText(insertElement[key]));    
                    }
                ...
                var updateUpdateSQL = function(updateElement, tableName, updateConditions){
                    ...
                    for (var key in updateElement) {
                        if(typeof(updateElement[key]) !== 'string'){
                            updateElement[key] = updateElement[key].toString();    
                        }
                        keyValues.push(be.db.quoteObject(key) + " = " + be.db.quoteText(updateElement[key]));
                    }
                    var sql =  "UPDATE " + be.db.quoteObject(tableName) + " SET " + keyValues.join(',') + " WHERE ";
                    var keyValues = [];
                    for (var key in updateConditions) {
                        if(typeof(updateConditions[key]) !== 'string'){
                            updateConditions[key] = updateConditions[key].toString();    
                        }
                        keyValues.push(be.db.quoteObject(key) + " = " + be.db.quoteText(updateConditions[key]));
                    };
                ...
                var processBitacora = function(hasError, status){
                    ...
                    if(procedureDef.bitacora.targetTable){
                        //falta un validador acá para que no rompa si está mal definido
                        var client;
                        be.getDbClient(req).then(function(cli){
                            client = cli;
                            var updateElement = {};
    ...
    for(var tableName in be.tableStructures){
        ...
        if(tableDef.sql.isTable){
            ...
            fieldsForSequences.forEach(function(fieldDef) {
                ...
                lines.push("CREATE SEQUENCE "+db.quoteObject(sequence.name)+" START "+db.quoteInteger(sequence.firstValue)+";");
                lines.push(
                    "ALTER TABLE "+db.quoteObject(tableName)+
                    " ALTER COLUMN "+db.quoteObject(fieldDef.name)+
                    (sequence.prefix==null
                    ?" SET DEFAULT nextval("+db.quoteText(sequence.name)+"::regclass);"
                    :" SET DEFAULT ("+db.quoteText(sequence.prefix)+" || nextval("+db.quoteText(sequence.name)+"::regclass)::text);"
                    )
                );
                lines.push('GRANT USAGE, SELECT ON SEQUENCE '+db.quoteObject(sequence.name)+' TO '+admin+';');
            });
            lines.push('');
        }
    }
