...
AppBackend.prototype.exts = {
    img: imgExts,
    normal: ['', 'js', 'map', 'html', 'css', ...imgExts, 'appcache', 'manifest', 'json', 'webmanifest', 'zip', ...fontExts]
};
        ...
        if(config.nodb){
            ...
            return Promise.resolve().then(function(){
                ...
                be.dbUserRolExpr=`(select ${be.db.quoteIdent(be.config.login.rolFieldName)} 
                    ...
                    where ${be.db.quoteIdent(be.config.login.rolFieldName)} = ${be.dbUserNameExpr})`
            }).then(function(){
                if(opts["dump-db"]){
            ...
            return Promise.all([
                ...
                fs.stat('client/login.jade').then(function(){
                    ...
                    be.config.login.plus.newPassPageServe=function(req,res,next){
                        return MiniTools.serveJade(loginFile.replace(/\blogin\b/,'new-password'),be.optsGenericForFiles(req).jade)(req,res,next);
                    };
                    be.config.login.plus.newPassPageServe2=function(req,res,next){
                        return MiniTools.serveJade(loginFile.replace(/\blogin\b/,'new-password-result'),be.optsGenericForFiles(req).jade)(req,res,next);
                    };
                    be.config.login.plus.newPassPageEnter=function(username, token){
                        return function(req,res,next){
                            return MiniTools.serveJade(loginFile.replace(/\blogin\b/,'new-pass-enter'),{...be.optsGenericForFiles(req).jade, username, token})(req,res,next);
                        }
                    };
                })
            ]);
        ...
        if(be.config.nologin){
            ...
            be.passwordChanger=function(req, username, oldPassword, newPassword, done) {
                ...
                be.getDbClient(req).then(function(cli){
                    ...
                    return client.query(sql, params).fetchOneRowIfExists();
                }).then(function(result){
                    client.done();
                    if(result.rowCount){
                        done(null,true);
                    }else{
                        done(null,false,{message: be.messages.server.oldPassDontMatch});
                    }
                }).catch(function(err){
...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    if(be.config.login.forget?.urlPath){
        ...
        mainApp.post(Path.posix.join(baseUrl,be.config.login.forget.urlPath), function(req,res,next){
            ...
            if(!req.body.email || !/@/.test(req.body.email)){
                ...
                var promise = be.inDbClient(req, async (client)=>{
                    ...
                    if(rows?.length>0){
                        ...
                        await be.sendMail({
                            to: req.body.email,
                            subject: be.message.newPassword.mailSubject,
                            html: be.messages.newPassword.mailHtml.replace(
                                '<a href=""></a>',
            ...
            console.log('aca 11')
            res.redirect(be.config.login.forget?.urlPathOk+'?result'+resultCode);
            console.log('aca 12')
            promise.catch(err=>{
        ...
        mainApp.post(Path.posix.join(baseUrl,'/new-pass-enter'), function(req,res,next){
            ...
            `, [req.body.token, now])
                ...
                res.redirect(be.config.login.noLoggedUrlPath);
    ...
    mainApp.get(Path.posix.join(baseUrl,'/img/login-logo-icon.png'), async function(req,res,next){
        console.log('entre a buscar la imagen')
        var buscar = [
...
AppBackend.prototype.dumpDbSchemaPartial = async function dumpDbSchemaPartial(partialTableStructures, opts = {}){
        ...
        try{
            ...
            if(tableDef.sql.isTable){
                ...
                tableDef.constraints.map(function(cons){
                    ...
                    consLines.push(
                        'alter table '+cualQuoteTableName+' add '+
                ...
                if(tableDef.sql.policies.enabled){
                    ...
                    ['all', 'select', 'insert', 'update', 'delete'].forEach((command)=>{
                        ...
                        if(polcom.using || polcom.check){
                            policyLines.push(`CREATE POLICY bp_${command} ON ${cualQuoteTableName} AS PERMISSIVE FOR ${command}`+
    ...
    return Promise.all(Object.keys(partialTableStructures).map(function(tableName,i){
        // TODO: buscar
        ...
        if(be.config.install['table-data-dir']){
