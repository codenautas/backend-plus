    ...
    return Promise.resolve().then(function(){
        ...
        if(config.nodb){
            ...
            return Promise.resolve().then(function(){
                ...
                if(be.config.db['log-last-error'] || be.config.devel || be.config.db['log-on-demand']){
                    if(be.config.devel || be.config.db['log-on-demand']){
                        if(be.config.db['log-until']){
                            try{
                                var ti=bestGlobals.timeInterval.iso(be.config.db['log-until'])
                                be.config.db['log-until'] = bestGlobals.datetime.now().add(ti);
                            }catch(err){
                                be.config.db['log-until'] = bestGlobals.datetime.iso(be.config.db['log-until']);
                            }
                        }else{
                            be.config.db['log-until'] = bestGlobals.datetime.now().add({ms:10*60*1000}); // 10 minutes
                        }
                        var logWaiter=Promise.resolve(); // para mandar todo en orden
                        console.log('SQL: log all until',be.config.db['log-until'].toYmdHms())
                        pg.log = function logAll(message, type){
                            if(bestGlobals.datetime.now()<=be.config.db['log-until']){
                                logWaiter=logWaiter.then(function(){
                                    if(be.config.db['log-results'] && (type=='ROW' || type=='RESULT')){
            ...
            mainApp.loginPlusManager.setValidatorStrategy(
                function(req, username, password, done) {
                    ...
                    be.getDbClient(req).then(function(cli){
                        ...
                        console.log('infoFieldList',infoFieldList)
                        return client.query(
...
AppBackend.prototype.addProcedureServices = function addProcedureServices(){ 
    ...
    return this.getProcedures().then(function(defs){
        ...
        likeAr(be.procedure).forEach(function(procedureDef){
            ...
            be.app[procedureDef.method]('/'+procedureDef.action, function(req, res, next){
                ...
                var processCoreFunction = function(){
                    return Promise.resolve().then(function(){
                        ...
                        return be.inTransaction(req, function(client){
                            ...
                            ['query', 'body', 'fields'].forEach(function(label){
                            });
                            return Promise.resolve().then(function(){
...
AppBackend.prototype.getClientSetupForSendToFrontEnd = function getClientSetupForSendToFrontEnd(req){
    ...
    clientSetup.procedures = likeAr(be.clientSetup.procedure).filter(function(procedureDef){
        return !procedureDef.roles || procedureDef.roles.indexOf(req.user[be.config.login.rolFieldName])>=0;
    }).array();
    return clientSetup;
};
...
AppBackend.prototype.getTables = function getTables(){
    ...
    if(be.staticConfig['client-setup']['grid-buffer'] && be.staticConfig['client-setup']['grid-buffer'] === 'idbx'){
...
AppBackend.prototype.mainPage = function mainPage(req, offlineMode){
    ...
    if(offlineMode && req && req.user){
        ...
        var width = be.config["client-setup"]["deviceWidthForMobile"] || 'device-width';
        var initialScale = ', initial-scale=' + (be.config["client-setup"]["initial-scale"]?be.config["client-setup"]["initial-scale"]: '1');
        var minimumScale = be.config["client-setup"]["minimum-scale"]?', minimum-scale=' + be.config["client-setup"]["minimum-scale"] :'';
        var maximumScale = be.config["client-setup"]["maximum-scale"]?', maximum-scale=' + be.config["client-setup"]["maximum-scale"] :'';
        var userScalable = be.config["client-setup"]["user-scalable"]?', user-scalable=' + be.config["client-setup"]["user-scalable"] :'';
        viewportAttrs={name:"viewport", content:"width="+width + initialScale + minimumScale + maximumScale+userScalable};
    }
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    if(be.config["client-setup"].menu){
        ...
        if(be.config['client-setup']['grid-buffer'] && be.config['client-setup']['grid-buffer'] === 'idbx'){
...
AppBackend.prototype.dumpDbSchemaPartial = function dumpDbSchemaPartial(partialTableStructures, opts = {}){
    ...
    return Promise.all(Object.keys(partialTableStructures).map(function(tableName){
        // TODO: buscar
        return locatePath(be.appStack.map(function(stackNode){
            ...
            if(/[-[]/.test(content[0])){
                ...
                lines.forEach(function(line){
                    var dataString="insert into "+db.quoteIdent(tableName)+" ("+
                        Object.keys(line).map(db.quoteIdent).join(', ')+
                        ') values ('+
