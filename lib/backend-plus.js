...
AppBackend.prototype.start = function start(opts){
    ...
    return Promise.resolve().then(function(){
        ...
        mainApp.use(
            be.config.server["base-url"]+'/',
            serveContent(Path.join(be.rootPath,'unlogged'),{
                allowedExts:be.exts.normal, 
                jade:{skin:be.config['client-setup'].skin}
            })
        );
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    var optsGenericForFiles={
        allowedExts:be.exts.normal,
        jade:{skin:skin, skinUrl:skinUrl}
    }
    this.app.get('/my-skin.js',MiniTools.serveText(`
        "use strict";
        myOwn.path.img='${imgUrl}';
        window.addEventListener('load', function(){
            document.body.addAttribute('my-skin', '${skin}')
        });
    `, 'application/javascript'));
    console.log('xxxxxxxxxxxxx',be.config['client-setup'].skins[skin]['local-path'])
    console.log(__dirname);
    console.log(this.rootPath);
    console.log(Path.join(__dirname,be.config['client-setup'].skins[skin]['local-path']));
    console.log(Path.resolve(__dirname,be.config['client-setup'].skins[skin]['local-path']));
    console.log(Path.resolve(be.config['client-setup'].skins[skin]['local-path']));
    this.app.use(serveContent(Path.resolve(be.config['client-setup'].skins[skin]['local-path']),{allowedExts:be.exts.img.concat(['css'])}));
