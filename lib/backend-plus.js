...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    var pathToNodeModulesSkin=Path.join(be.config.server.skins[skin]['local-path'],'unlogged');
    ...
    console.log('Donde hay que buscar',pathToNodeModulesSkin);
    mainApp.use(baseUrl+(skin?'/'+skin:''),serveContent(pathToNodeModulesSkin,optsGenericForFiles));
    mainApp.use(baseUrl,serveContent(Path.join(be.rootPath,'unlogged'),optsGenericForFiles));
};
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    this.app.use(slashSkin,serveContent(Path.resolve(be.config.server.skins[skin]['local-path']),{allowedExts:be.exts.img.concat(['css'])}));
