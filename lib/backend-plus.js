        ...
        pregunta:{
            completar: function completar(celda, be, idFormulario, celdasAgregadas){
                ...
                if(celda.typeInfo.typeName=='multiple'){
                    //console.log('============== veo multiple',celda)
                    // Agregué en 12 renglones más abajo el grupo a la opción múltiple
                    celda.tipo='texto';
                    celda.opciones.forEach(function(opcion){
                        celdasAgregadas.push({
                            tipo: 'pregunta',
                            subtipo: 'multiple',
        ...
        _.forEach(estructura.formularios, function(formulario, idFormulario){
            _.forEach(formulario.celdas, function(celda, indexCelda){
                ...
                while(celdasAgregadas.length){
                    formulario.celdas.splice(indexCelda+1,0,celdasAgregadas.pop());
                }
            });
        });
        return estructura;
