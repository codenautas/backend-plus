...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    be.clientIncludesCompleted().filter(x => x.module).forEach(function (moduleDef) {
        ...
        try {
            ...
            if(moduleDef.module=='rel-enc'){
                console.log('############# por servir xxxxxxxxx',baseLib,resolve_module_dir(moduleDef.module, moduleDef.modPath),moduleDef.module, moduleDef.modPath);
                console.log('############# resolve puro xxxxxxxxx',require.resolve(moduleDef.module),require_resolve(moduleDef.module),resolve_module_dir(moduleDef.module));
            }
...
AppBackend.prototype.clientIncludesCompleted = function clientIncludesCompleted(req, hideBEPlusInclusions) {
    ...
    return list.map(inclusion =>{
        if(/rel-enc/.test(inclusion.module) || /rel-enc/.test(inclusion.file)){
            console.log('xxxxxxxxxxxxx',inclusion);
            console.log(process.cwd())
            console.log(require_resolve(inclusion.module,rPaths))
        }
        ...
        if(tableDef.sql.isTable){
            ...
            tableDef.foreignKeys.map(function(fk){
                fkLines.push('alter table '+db.quoteIdent(tableDef.name)+' add constraint '+' '+
                    db.quoteIdent(fk.consName?fk.consName:(tableDef.alias||tableDef.name)+' '+fk.references+' REL')+' '+
                    'foreign key ('+
