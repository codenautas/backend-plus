        ...
        be.procedures.forEach(function(procedureDef){
            ...
            be.app[procedureDef.method]('/'+procedureDef.action, function(req, res){
                ...
                return Promise.resolve().then(function(){
                    ...
                    throw err1;
                    return context.client.query("ROLLBACK").execute().then(function(){context.client.done();},function(err2){
                        context.client.done();
                        console.log("Error in database when trying to "+procedureDef.action,err1," Error trying to rollback ",err2);
                        throw err;});
                }).then(function(result){
                    context.client.done();
                    if(result===undefined){
...
AppBackend.prototype.addUnloggedServices = function addUnloggedServices(mainApp, baseUrl){
    ...
    skinPaths.concat(unloggedPaths).forEach(function(path){
        ...
        console.log('xxxxxx Donde hay que buscar con skin/',baseUrl+(skin?'/'+skin:''),pathToNodeModulesSkin);
        mainApp.use(baseUrl+(skin?'/'+skin:''),serveContent(pathToNodeModulesSkin,optsGenericForFiles));
    });
    unloggedPaths.forEach(function(path){
        var pathToNodeModulesSkin=Path.join(path, 'unlogged');
        console.log('xxxxxx Donde hay que buscar sin skin/',baseUrl,pathToNodeModulesSkin);
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    var optsGenericForFiles={
        ...
        jade:{
            skin:skin, 
            skinUrl:skinUrl,
            msgs: changing(be.messages.unlogged, be.messages),
    ...
    console.log('xxxxxxxxxxxx DONDE',slashSkin,Path.join(be.config.server.skins[skin]['local-path'],skin));
    ...
    console.log('xxxxxxxxxxxx DONDE',slashSkin,Path.join(be.rootPath,'skins',skin));
    ...
    console.log('xxxxxxxxxxxx jonde',Path.join(__dirname,'../for-client'));
    ...
    this.app.use('/keep-alive', function(req, res, next){
        try{
            if(new Date().getTime() - req.session.lastNonKeepAlive > be.config.login.keepAlive*1000){
                console.log('xxxxxxxxxxxxx LOGOUT decidido en keep-alive');
                req.logout();
...
AppBackend.prototype.dumpDbTables = function dumpDbTables(){
    ...
    return Promise.all(Object.keys(be.tableStructures).map(function(tableName){
        ...
        return Promise.all(['prepare.sql','adapt.sql'].map(function(fileName){
