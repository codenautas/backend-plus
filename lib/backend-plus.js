...
AppBackend.prototype.exts = {
    ...
    normal: ['', 'js', 'map', 'html', 'css', 'jpg', 'jpeg', 'png', 'ico', 'gif', 'appcache']
};
...
function resolve_module_dir(moduleName,path){
    return Path.join(Path.dirname(require_resolve(moduleName)),path||'');
}
...
AppBackend.prototype.mainPage = function mainPage(req, offlineMode){
    ...
    if(offlineMode && req && req.user){
        attr.manifest="ext/offline.appcache";
    }
    return html.html(attr,[
        ...
        html.body({'app-version':packagejson.version},[
            ...
            }).concat([html.script({src:'menu.js'})]))
        ]),
    ]);
}
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    if(be.config["client-setup"].menu){
        ...
        if(be.config['client-setup']['grid-buffer'] && be.config['client-setup']['grid-buffer'] === 'idbx'){
            be.app.get('/ext', function(req,res,next){
                var mainPage = be.mainPage(req, true);
                fs.writeFile('client/ext/ext.html', mainPage.toHtmlText({pretty:true}))
                return MiniTools.serveText(mainPage.toHtmlDoc(),'html')(req,res,next);
                //return MiniTools.serveFile(Path.join(be.rootPath+'/client/ext','ext.html'))(req,res,next);
            });
        }
    }
    ...
    [Path.resolve(be.rootPath,'src/client')].forEach(function(path){
        be.app.use('/src/client', function(req,res,next){
...
AppBackend.prototype.dumpDbSchemaPartial = function dumpDbSchemaPartial(partialTableStructures, opts = {}){
    ...
    if(be.config.install.dump["admin-can-create-tables"]){
        linesCreate.push("grant create on schema "+schema+' to '+admin+';');
    }
    linesCreate.push('');
        ...
        try{
            ...
            if(tableDef.sql.isTable){
                ...
                if(tableDef.sql.isReferable){
                    rights.push('references')
                }
                rights=rights.join(', ')
                if(rights){
                    lines.push('grant '+rights+' on '+db.quoteIdent(tableDef.sql.tableName)+' to '+admin+';');
                }
                tableDef.foreignKeys.map(function(fk){
                    fkLines.push('alter table '+db.quoteIdent(tableDef.sql.tableName)+' add constraint '+' '+
                        db.quoteIdent(fk.consName?fk.consName:(tableDef.alias||tableDef.sql.tableName)+' '+fk.references+' REL')+' '+
                        'foreign key ('+
    ...
    return Promise.all(Object.keys(partialTableStructures).map(function(tableName){
        // TODO: buscar
        var theTableFileName=be.rootPath+'/install/'+tableName+'.tab';
        return locatePath(be.appStack.map(function(stackNode){
            return Path.join(stackNode.path,'../install',tableName+'.tab');
            ...
            return fs.readFile(theTableFileName, {encoding:'UTF8'});
