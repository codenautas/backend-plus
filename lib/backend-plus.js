            ...
            completar: function completar(celda, be, idFormulario, celdasAgregadas){
                ...
                if(celda.typeInfo.typeName=='multiple'){
                    ...
                    celda.opciones.forEach(function(opcion){
                        var variable=celda.pregunta+'_'+opcion.opcion;
                        var celdaNueva={
                            tipo: 'pregunta',
                            subtipo: 'multiple',
        ...
        _.forEach(estructura.formularios, function(formulario, idFormulario){
            ...
            formulario.celdas = formulario.celdas.reduce(function(nuevoArreglo ,celda){
                ...
                while(celdasAgregadas.length){
                    nuevoArreglo.push(celdasAgregadas.pop());
                }
                return nuevoArreglo;
            }, []);
        });
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    this.app.use(/* baseUrl, */ MiniTools.serveJade(Path.resolve(this.rootPath,'client'),true));
    this.app.use(/* baseUrl, */ MiniTools.serveStylus(Path.resolve(this.rootPath,'client'),true));
    this.app.use(/* baseUrl, */ extensionServeStatic(Path.resolve(this.rootPath,'client'),{staticExtensions:'js'}));
    this.app.use(/* baseUrl, */ extensionServeStatic('./node_modules/lodash', {staticExtensions:'js'}));
    this.app.use(/* baseUrl, */ extensionServeStatic('./node_modules/require-bro/lib', {staticExtensions:'js'}));
    ...
    this.app.use(/* baseUrl, */ extensionServeStatic('./node_modules/regexplicit', {staticExtensions:'js'}));
}
