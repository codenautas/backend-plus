    ...
    return this.getProcedures().then(function(defs){
        ...
        be.app.get('/client-setup',function(req, res, next){
            ...
            if(be.config["client-setup"].menu===true){
                clientSetup=changing(clientSetup, be.getMenu(be.getContext(req)));
            }
        ...
        be.procedures.forEach(function(procedureDef){
            ...
            be.app[procedureDef.method]('/'+procedureDef.action, function(req, res){
                ...
                return Promise.resolve().then(function(){
                    ...
                    'query,body,fields'.split(',').forEach(function(label){
                        // console.log('xxxxxx req.'+label,req[label]);
                    });
            ...
            if(procedureDef.withFastWindow && procedureDef.method!='GET'){
                be.app.get('/'+procedureDef.action, function(req, res){
                    be.addLoggedServices();
                    var baseLib = be.config.server["base-url"]+'/lib/node_modules/';
                    var buttonAjax = html.button({id:'button-formulario-ajax-procedures'},'Enviar');
                    var table=html.body([html.form({id:"formulario-ajax-procedures"},[html.table(
                        procedureDef.parameters.map(function(params){
                            return html.tr([
                                html.td([html.div(params.name)]),
                                html.td([html.input()])
                            ])
                        }).concat(html.tr([html.td([buttonAjax])]))
                    )])].concat(html.script({src:'my-debugger.js'})).concat(be.clientModules().scripts.map(function(modulos){
                        return html.script({src:'../'+modulos.src});
                    })));
                    res.end(table.toHtmlText())
                })
            }
        });
    });
...
AppBackend.prototype.clientModules = function clientModules(){
    var be=this;
    var scriptList=[
        {src:'lib/js-yaml.js'                                       },
...
AppBackend.prototype.mainPage = function mainPage(){
    var be=this;
    return html.html({lang: "en"}, [
        html.head([
            ...
            html.link({href: "css/my-menu.css", rel: "stylesheet"}),
        ...
        html.body([
            html.div({id: "total-layout"}, [
                html.img({src: "main-loading.png"}),
                html.div(be.messages.unlogged.loading)
            ]),
            html.div({id: "total-scripts"}, be.clientModules().scripts.map(function(scriptDef){
...
AppBackend.prototype.addLoggedServices = function addLoggedServices(){
    ...
    if(be.config["client-setup"].menu){
        be.app.get('/menu',MiniTools.serveText(be.mainPage().toHtmlDoc(),'html'));
    }
    this.app.get('/my-skin.js',MiniTools.serveText(`
        "use strict";
    ...
    this.app.use(slashSkin,serveContent(Path.join(be.config.server.skins[skin]['local-path'],skin),{allowedExts:be.exts.img.concat(['css'])}));
    this.app.use(slashSkin,serveContent(Path.join(be.rootPath,'skins',skin),{allowedExts:be.exts.img.concat(['css'])}));
    this.app.use(serveContent(Path.resolve(this.rootPath,'client'),optsGenericForFiles));
    this.app.use(serveContent(Path.join(__dirname,'../for-client'),optsGenericForFiles));
...
AppBackend.prototype.dumpDbCreateDatabase = function dumpDbCreateDatabase(){
    ...
    // GRANT CONNECT ON DATABASE soft_skin_db TO soft_skin_admin;
...
AppBackend.prototype.dumpDbSchema = function dumpDbSchema(){
    ...
    return Promise.all(Object.keys(be.tableStructures).map(function(tableName){
        ...
        return Promise.all(['prepare.sql','pre-adapt.sql','adapt.sql'].map(function(fileName){
