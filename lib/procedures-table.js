...
ProcedureTables = [
    ...
    },
    {
        ...
        coreFunction:function(context, parameters){
            var be=context.be;
            var defTable=be.tableStructures[parameters.table](context);
            if(parameters.status=='new'){
                if(!defTable.allowInserts){
                    throw changing(new Error("Inserts not allowed"),{status:403});
                }
            }else{
                if(!defTable.allowUpdates){
                    throw changing(new Error("Updates not allowed"),{status:403});
                }
            }
