    ...
    },
    {
        ...
        coreFunction:async function(context, parameters, files, opts){
            ...
            while(updates.length){
                ...
                defTable.primaryKey.forEach(function(name,i){
                    primaryKeyValueObject[name]=primaryKeyValues[i]=main?parameters.primaryKeyValues[i]:coalesce(parameters.newRow[name],parameters.oldRow[name]);
                });
            ...
            return Promise.resolve().then(async function(){
                ...
                var testCandidateColumnAndAddIt=function testCandidateColumnAndAddIt(fieldName, iColumn, cellAddress){
                    ...
                    if(fieldName){
                        ...
                        if(defField && defField.inTable!==false && (defField.allow.import || defField.allow.insert && (defField.allow.update || defField.isPk))){
                ...
                return readerFunction.then(function(rowsAndOtherFieldsNames){
                    ...
                    rowsForUpsert.forEach(function(newRow, lineNumber){
                        ...
                        var slicers={};
                        fields.forEach(function(field){
                            var value=field.name in newRow?newRow[field.name]:(
                                'defaultValue' in field?field.defaultValue:(
                                    'specialDefaultValue' in field?my.specialDefaultValue[fieldDef.specialDefaultValue](field.name, {row:{}}, {row:previousRow}):null
                                )
                            );
