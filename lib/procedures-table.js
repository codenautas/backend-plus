        ...
        coreFunction:function(context, parameters, files){
            ...
            return Promise.resolve().then(function(){
                ...
                var addFieldToOthers = function addFieldToOthers(othersArray, field, value){
                    ...
                    othersArray.push({name: otherFieldName.name, value: value || ''});  
                };
                ...
                return readerFunction.then(function(rowsAndOtherFieldsNames){
                    ...
                    if(defTable.JSONFieldForOtherFields){
                        ...
                        otherFieldNames.forEach(function(otherField){
                            var newRow = {};
                            newRow[defTable.registerImports.fieldNames.tableName] = parameters.table;
                            newRow[defTable.registerImports.fieldNames.fieldName] = otherField.name;
                            newRow[defTable.registerImports.fieldNames.fieldIndex] = otherField.index;
                            newRow[defTable.registerImports.fieldNames.originalFileName] = files[0].originalFilename;
                            newRow[defTable.registerImports.fieldNames.serverPath] = files[0].path;
                            newRow[defTable.registerImports.fieldNames.lastUpload] = timestamp;
                            promiseChain = promiseChain.then(function(){
                                return be.procedure['table/save-record'].coreFunction(
                                    context,{
                                        table: otherFieldsTableDef.name, 
                                        primaryKeyValues: otherFieldsTableDef.primaryKey,
                                        newRow,
                                        oldRow:[],
                                        status:'update',
                                        insertIfNotUpdate:true,
