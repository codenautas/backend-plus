...
ProcedureTables = [
    ...
    },
    {
        ...
        coreFunction:function(context, parameters){
            ...
            var fixedClausule=parameters.fixedFields.map(function(pair, iPair){
                ...
                return ' AND '+be.db.quoteIdent(defTable.alias)+'.'+be.db.quoteIdent(pair.fieldName)+ " = $" + queryValues.length;
            }).join("");
            return context.client.query(
        ...
        coreFunction:function(context, parameters){
            ...
            return Promise.resolve().then(function(){
                ...
                if(defTable.sql.from && row){
                    return be.queryValuesOfUniqueRow(context, defTable, row);
                }else{
                    return result;
                }
            }).then(function(result){
    ...
    },
    {
        ...
        coreFunction:function(context, parameters){
            if(context.user[context.be.config.login.rolFieldName]==='admin' && context.user[context.be.config.login.userFieldName]!=parameters.user){
