...
ProcedureTables = [
    ...
    },
    {
        ...
        coreFunction:async function(context, parameters, files, opts){
            ...
            for(var name in parameters.newRow){
                ...
                var updateTarget;
                if(defField.table && defField.table!=defTable.name){
                    updateTarget=otherUpdates.tables[defField.table]=otherUpdates.tables[defField.table]||emptyUpdater();
                }else if(defField.tableName && defField.tableName!=(defTable.tableName||defTable.name)){
                ...
                updateTarget.values.push(parameters.newRow[name]);
            }
            ...
            var updates=likeAr(otherUpdates.tables).map(function(data,name){
                ...
                return {defTable:changing(tableDef, ((defTable.sql.otherTableDefs||{})[name])||{}), ...data};
            }).array().concat(likeAr(otherUpdates.tableNames).map(function(data,name){
                return {defTable:defTable.otherTableDefs[name],main:true, ...data};
            }).array()).concat(plainUpdate.values.length?[changing({defTable},plainUpdate)]:[]);
            ...
            while(updates.length){
                ...
                defTable.primaryKey.forEach(function(name,i){
                    primaryKeyValueObject[name]=primaryKeyValues[i]=main?parameters.primaryKeyValues[i]:coalesce(parameters.primaryKeyValues[i],parameters.newRow[name],parameters.oldRow[name]);
                });
            ...
            if((!defTable || parameters.status!='new') 
                && (!defTable || primaryKeyValues.length!=defTable.primaryKey.length)
                && (parameters.masive && primaryKeyValues.length>0)
            ){
                return {masive:true};
            }
            var insertOrUpdate=result.command;
            if(defTable.sql.from && result.row && (!opts || !opts.forImport)){
                var rowFrom = plainUpdate.values.length?result.row:primaryKeyValueObject;
                result = await be.queryValuesOfUniqueRow(context, defTable, rowFrom);
            }
    ...
    },
    {
        ...
        coreFunction:async function(context, parameters, files){
            async function recreateConstraints(tableDef, force){
                return Promise.all(tableDef.foreignKeys.filter(fk=>fk.forceDeferrable).map(async function(fk){
                    var {consName, clause} = be.dumpFkConstraint(fk, tableDef, force);
                    await context.client.query(`ALTER TABLE ${be.db.quoteIdent(tableDef.tableName)} DROP ${be.db.quoteIdent(consName)}`);
                    await context.client.query(`ALTER TABLE ${be.db.quoteIdent(tableDef.tableName)} ADD ${clause}`);
                }))
            }
            var be=context.be;
                    ...
                    readerFunction=fs.readFile(files[0].path).then(function(content){
                        ...
                        for(var iRow=range.s.r+1; iRow<=range.e.r; iRow++){
                            ...
                            fields.forEach(function(field){
                                ...
                                if(value=='NA' || value===''){  // OJO generalizar el NA
                                    value=null;
                                }
                                if(field.defaultForOtherFields){
                                    addFieldToOthers(othersArray, field, value);
