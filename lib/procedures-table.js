...
var notnull = x =>{
    if (x==null) throw new Error("not null expected");
    return x;
};
...
ProcedureTables = [
    {
        ...
        coreFunction:async function(context, parameters){
            ...
            var sanitizeField = ({generatedAs, defaultDbVAlue, nameForUpsert, ...def})=>def;
            return {
                ...struct,
                sql: undefined,
                fields: struct.fields.map(sanitizeField),
                field: likeAr.beingArray(struct.field).map(sanitizeField).plain()
            };
        }
    },
    {
        ...
        *
        * @param {*} context
        * @param {{table:string, fixedFields:{fieldName:string, value:any, range?:string, until?:string}[], paramfun:Record<string, string>, pick:string, retrieveIgnoringWhere:boolean}} parameters
        */
        async function tableDatum(context, parameters){
            ...
            parameters.fixedFields.forEach(function(pair, iPair){
                ...
                if(defTable.sql && defTable.sql.fields[fieldName] && defTable.sql.fields[fieldName].expr){
                    // TODO: ver si esto se está usando y cómo
            ...
            if(specialFixedClause.length){
                sql="SELECT *\n FROM ("+sql+") x\n WHERE "+specialFixedClause.join(" AND ");
            }
    ...
    },
    {
        ...
        coreFunction:async function(context, parameters, files, opts){
            ...
            function emptyUpdater(name){ return {name, fieldNames:[], values:[]}};
            var plainUpdate=emptyUpdater(mainDefTable.name);
            /** @typedef { {name:string, fieldNames:string[],values:any[],main?:boolean} } TableUpdater */
            /** @type { { tables:Record<string, TableUpdater>, tableNames:Record<string, TableUpdater> } } */
            var otherUpdates={
                tables:{},
                tableNames:{}
            };
            ...
            for(var name in parameters.newRow){
                ...
                if(defField.table && defField.table!=mainDefTable.name){
                    updateTarget=otherUpdates.tables[defField.table]=otherUpdates.tables[defField.table]||emptyUpdater(mainDefTable.sql?.otherTableDefs?.[defField.table].name ?? defField.table);
                }else if(defField.tableName && defField.tableName!=(mainDefTable.tableName||mainDefTable.name)){
                    updateTarget=otherUpdates.tableNames[defField.table]=otherUpdates.tableNames[defField.table]||emptyUpdater(defField.table);
                }else{
                    updateTarget=plainUpdate;
                }
                updateTarget.fieldNames.push(defField.nameForUpsert ?? name);
                var value=(
            ...
            var updatesOthers=likeAr(otherUpdates.tables).map(function(data,name){
                if(be.tableStructures[data.name] == null){
                    console.log("not found ",be.tableStructures[data.name],data.name)
                }
                var tableDef=be.tableStructures[data.name](context);
                ...
                if(files[0].originalFilename.match(/\.(tab|txt)$/)){
                    readerFunction=fs.readFile(files[0].path,{encoding:'utf8'}).then(function(content){
                        ...
                        while(heading.length<2 && separatorCandidate.length>1){
                            separator = notnull(separatorCandidate.shift());
                    ...
                    readerFunction=fs.readFile(files[0].path).then(function(content){
                        ...
                        var range = XLSX.utils.decode_range(notnull(ws['!ref']));
                        doing="detecting fields";
                    ...
                    for(let newRow of rowsForUpsert){
                        ...
                        for(let field of fields){
                            ...
                            if(field.isPk){
                                ...
                                if(value==null && !field.specialValueWhenInsert){
