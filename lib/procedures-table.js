    ...
    },
    {
        ...
        coreFunction:async function(context, parameters){
            ...
            var results = await Promise.all(tableDefArray.map(
                function(tableDef){
                    var from = tableDef.sql.from?tableDef.sql.from:`${db.quoteIdent(tableDef.sql.tableName)}`;
                    return context.client.query(
                        `select * from ` + from + `
                            where ${pk_clausule}`,
            ...
            var rowsByTable = results.map(result=>result.rows);
            var promiseChain=Promise.resolve();
            tableDefArray.forEach(function(tableDef,i){
                rowsByTable[i].forEach(function(row){
