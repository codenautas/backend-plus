    ...
    },
    {
        ...
        coreFunction:async function(context, parameters, files, opts){
            ...
            var updates=likeAr(otherUpdates.tables).map(function(data,name){
                ...
                return {defTable:mainDefTable.otherTableDefs[name],main:true, ...data};
            }).array()).concat(plainUpdate.values.length?[changing({defTable:mainDefTable},plainUpdate)]:[]);
            ...
            if(mainDefTable.sql.from && result.row && (!opts || !opts.forImport)){
                ...
                var rowFrom = likeAr.toPlainObject(mainDefTable.primaryKey,parameters.primaryKeyValues);
                        ...
                        fields.forEach(function(field){
                            var value=field.name in newRow?newRow[field.name]:(
                                'defaultValue' in field?field.defaultValue:(
                                    'specialDefaultValue' in field?new Error("my.specialDefaultValue[defField.specialDefaultValue](name, {row:{}}, {row:previousRow})"):null
                                )
                            );
