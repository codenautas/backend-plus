        ...
        coreFunction:function(context, parameters){
            ...
            var deleteOneRow=function(row){
                ...
                defTable.fields.forEach(function(fieldDef){
                    console.log('fieldDef.inexactNumber',fieldDef);
            ...
            return Promise.all(parameters.rowsToDelete.map(deleteOneRow)).then(function(){
                return context.client.query(
                    "SELECT count(*) remaining_record_count FROM "+be.db.quoteObject(defTable.name)
                ).fetchUniqueRow().then(function(result){
                    return result.row;
                });
            });
            ...
            return Promise.resolve().then(function(){
                ...
                if(files[0].originalFilename.match(/\.(tab|txt)$/)){
                    readerFunction=fs.readFile(files[0].path,{encoding:'UTF8'}).then(function(content){
                        ...
                        heading.forEach(function(fieldName, iColumn){
                            var cellAddress=iColumn;
                            var cellOfFieldName=heading[cellAddress];
                            if(cellOfFieldName){
                                var fieldName=cellOfFieldName.v.toLowerCase().trim();
                                if(fieldName){
                                    var defField=defTable.field[fieldName];
                                    if(!defField && !be.config.skipUnknownFieldsAtImport){
                                        throw new Error(be.messages.columnDoesNotExistInTable.replace('$1', cellAddress));
                                    }
                        ...
                        var rowsForUpsert=lines.slice(1).map(function(line){
                            ...
                            fields.forEach(function(field){
                                var value=row[field.column]||null;
                    ...
                    readerFunction=fs.readFile(files[0].path).then(function(content){
                        ...
                        for(var iColumn=range.s.c; iColumn<=range.e.c; iColumn++){
                            ...
                            if(cellOfFieldName){
                                if(typeof cellOfFieldName.v !== 'string'){
                                    throw new Error(be.messages.importColumnDoesNotHasStringValue.replace('$1', cellAddress));
                                }
                                var fieldName=cellOfFieldName.v.toLowerCase().trim();
                                if(fieldName){
                                    var defField=defTable.field[fieldName];
                        ...
                        for(var iRow=range.s.r+1; iRow<=range.e.r; iRow++){
                            ...
                            fields.forEach(function(field){
                                ...
                                console.log("field.column ",field.iColumn);
                                var value=cell?cell.v||null:null;
                                newRow[field.name]=value;
                            });
                ...
                return readerFunction.then(function(rowsForUpsert){
