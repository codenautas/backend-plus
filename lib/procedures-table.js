                ...
                if(files[0].originalFilename.match(/\.(tab|txt)$/)){
                    readerFunction=fs.readFile(files[0].path,{encoding:'UTF8'}).then(function(content){
                        ...
                        heading.forEach(function(fieldName, iColumn){
                            ...
                            if(cellOfFieldName){
                                var fieldName=cellOfFieldName.toLowerCase().trim();
                                if(fieldName){
                                    var defField=defTable.field[fieldName];
                                    if(!defField && !be.config.skipUnknownFieldsAtImport){
                                        console.log('xxxxxxxxxxxxx defField & import',fieldName,defField);
                        ...
                        rowsForUpsert.slice(0,10).forEach(function(row){
                            console.log(row);
                        });
                ...
                return readerFunction.then(function(rowsForUpsert){
                    ...
                    rowsForUpsert.forEach(function(newRow){
                        ...
                        if(!skip){
                            ...
                            // console.log('xxxxxxxx skiped',newRow);
                        }
                    });
                    return promiseChain;
                }).then(function(){
