    ...
    },
    {
        ...
        coreFunction:function(context, parameters){
            ...
            parameters.fixedFields.forEach(function(pair, iPair){
                ...
                var exprClausule=' AND '+be.db.quoteIdent(alias)+'.'+be.db.quoteIdent(fieldName) + " = $" + queryValues.length
                if(defTable.sql && defTable.sql.fields[fieldName] && defTable.sql.fields[fieldName].expr){
                    specialFixedClause.push(be.db.quoteIdent(fieldName) + " = $" + queryValues.length)
                }else{
